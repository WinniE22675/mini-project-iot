...
# ===============================
# NEW PART: Load dataset
# ===============================
DATA_FILE = "smart_lighting_dataset_2024.csv"
df = pd.read_csv(DATA_FILE)

# Convert DataFrame to list of dicts for easy iteration
records = df.to_dict(orient="records")

# ===============================
# Modified publish function
# ===============================
async def publish_sensor(sensor_config, records):
    idx = 0
    while True:
        try:
            ...
            async with Client(MQTT_BROKER) as client:
                ...
                while idx < len(records):
                    ...
                    # Construct payload (send everything from CSV row)
                    sensor_data = {
                        "id": sensor_config["id"],
                        "name": sensor_config["name"],
                        "place_id": sensor_config["place_id"],
                        "payload": row  # directly send the row dict
                    }

                    # Encode message and publish
                    message = json.dumps(sensor_data, default=str)  # default=str for timestamp
                    ...
                    idx += 1
                    await asyncio.sleep(1)  # adjust publish interval as needed

                logging.info(f"[{sensor_config['name']}] All data published. Restarting from top.")
                idx = 0  # loop back if you want continuous streaming
...
# ===============================
async def main():
    # example: one sensor publisher that streams the dataset
    sensor_configs = [{
        "id": "000000",
        "name": "iot_sensor_dataset",
        "place_id": "000000"
    }]
    await asyncio.gather(*(publish_sensor(config, records) for config in sensor_configs))
